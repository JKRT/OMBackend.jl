...
function PendulumModel(tspan = (0.0, 1.0))
  ...
  vars = ModelingToolkit.@variables((x(t), y(t), phi(t), phid(t), vy(t), vx(t)))
  der = Differential(t)
  eqs = [
    der(x) ~ vx,
    der(y) ~ vy,
    der(phi) ~ phid,
    der(phid) ~ -g * (L^-1) * sin(phi),
    0 ~ x - L * sin(phi),
    0 ~ y - -(L * cos(phi)),
  ]
...
PendulumModel_problem = PendulumModel()
function PendulumSimulate(tspan)
  return solve(PendulumModel_problem, tspan = tspan)
end
function PendulumSimulate(tspan = (0.0, 1.0); solver = Tsit5())
